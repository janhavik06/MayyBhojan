...
export default function AuthFlow({ mode = "login", setLoggedIn }) {
  ...
  const handleLogin = () => {
    const storedUser = JSON.parse(localStorage.getItem("maybhojan_user"));
    ...
    if (
      ...
      if (storedUser.role === "cook") navigate("/cook/login");

      if (storedUser.role === "delivery") navigate("/delivery");
